        ...
        if (filter != null) {
            ServiceTracker<S, T> tracker = new ServiceTracker<S, T>(context, getOrCreateFilter(filter), null) {
                ...
                public T addingService(ServiceReference<S> reference) {
                    ...
                    LOGGER.info("STH Test: Obtained service dependency: '" + filter + "' = '" + s + "'");
                    return creator.apply(s);
                }
                @Override
                public void removedService(ServiceReference<S> reference, T service) {
                    LOGGER.info("STH Test: Lost service dependency: '" + filter + "' = '" + service + "'");
            ...
            tracker.open();
            if (tracker.isEmpty()) {
                LOGGER.info("STH Test: Waiting for service dependency: '" + filter + "'");
            }
            return tracker;
        } else {
